 ...
 */
public class ImprovedByStreamApi {
    ...
    public static void main(String[] args) {
        ...
        //限定出大于25岁的用户
        list.stream().filter(user -> user.getAge() > 25).forEach(user -> System.out.println(user));
        System.out.println("------------------------------");
        //限定出薪水大于1000的用户
        list.stream().filter(user -> user.getSalary() > 1000).forEach(user -> System.out.println(user));
    }
}
